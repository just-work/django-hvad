...
RAISE_IF_NOT_TRANSLATION = getattr(settings, 'RAISE_IF_NOT_TRANSLATION', True)
    ...
    def translation(self, instance):
        ...
        if translation is None:
            try:
                ...
                if RAISE_IF_NOT_TRANSLATION:
                    raise self._NoTranslationError(
                        'Accessing a translated field requires that the '
                        'instance has a translation loaded, or a valid '
                        'translation in current language (%s) loadable from '
                        'the database' % get_language())
                translation = get_translation(instance, settings.LANGUAGE_CODE)
